...
exports.store = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const { contactId, queueId, status, userId } = req.body;
    const ticket = yield CreateTicketService_1.default({ contactId, status, userId, queueId });
...
exports.update = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (ticketData.transf) {
        ...
        if (ticketData.userId) {
            const nome = yield ShowUserService_1.default(ticketData.userId);
            const msgtxt = "*Mensagem Automática:*\nSeu *Atendimento* foi transferido para  O(a) consultor(a) *" + nome.name + "* irá atendê-lo(a). Aguarde um momento por gentileza!";
            yield SendWhatsAppMessage_1.default({ body: msgtxt, ticket });
        }
        else {
            const { name } = yield ShowQueueService_1.default(ticketData.queueId);
            const msgtxt = "*Mensagem Automática:*\nChat tranferido para o departamento *" + name + "*.\nAguarde um momento, iremos atende-lo(a)!";
            yield SendWhatsAppMessage_1.default({ body: msgtxt, ticket });
        }
    }
